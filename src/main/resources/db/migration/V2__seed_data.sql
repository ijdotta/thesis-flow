-- V2: Seed baseline reference data for a fresh schema
...
INSERT INTO project (public_id, title, type, initial_submission, completion, career_id, application_domain_id, created_at, updated_at) VALUES
 ('99999999-9999-9999-9999-999999999999','Sample Thesis Project','THESIS',CURRENT_DATE,NULL,(SELECT id FROM career WHERE public_id='aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaa1'),(SELECT id FROM application_domain WHERE public_id='bbbbbbbb-bbbb-bbbb-bbbb-bbbbbbbbbbb1'),CURRENT_TIMESTAMP,CURRENT_TIMESTAMP),
 ('88888888-8888-8888-8888-888888888888','Web Platform Final Project','FINAL_PROJECT',CURRENT_DATE,NULL,(SELECT id FROM career WHERE public_id='aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaa1'),(SELECT id FROM application_domain WHERE public_id='bbbbbbbb-bbbb-bbbb-bbbb-bbbbbbbbbbb2'),CURRENT_TIMESTAMP,CURRENT_TIMESTAMP)
ON CONFLICT (public_id) DO NOTHING;
